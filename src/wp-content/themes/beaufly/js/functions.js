...
( function( $ ) {
	...
	function _sale_nofication_start(){
		if($(".sale-nofication").length){
			...
			$.ajax({
				url: beaufly_ajax.ajaxurl,
				type: 'POST',
				dataType: 'json',
				data: {
					action : "beaufly_time_nofication_ajax",
					id_product : id_product,
					security : beaufly_ajax.ajax_nonce
				},
	...
	function _beaufly_accordion_menu(){	
	...
	function _headercategories(){
		...
		$('.beaufly-close',$menu_categories).on( "click", function() {
			$menu_categories.removeClass('active');
			return false;
		});		
		//Navigation right			
	...
	function _canvasrightNavigation(){
		...
		$('.beaufly-close',$wpbingo_menu_right).on( "click", function() {
			$wpbingo_menu_right.removeClass('active');
			return false;
		});		
		//Navigation right			
		...
		$(".input-search",$element).on("keydown", function() {
			...
			if(character.length >= 2){
				...
				$.ajax({
					...
					dataType: 'json',
					data: {
						action : "beaufly_search_products_ajax",
						character : character,
						limit : limit,
						category : category,
						security : beaufly_ajax.ajax_nonce
	...
	function _load_360_view_product(){
		...
		$(".beaufly-360-button").on( "click", function() {
	...
	function _after_add_to_cart(){ 
		...
		$( _body ).on( 'added_to_cart', function(){
			if(wd_width >= 991){
				var $element = $(".beaufly-topcart-desktop");
			}else{
				var $element = $(".beaufly-topcart-mobile");
			}
	...
	function _ajax_cart_header(){
		var wd_width = _window.width();
		if(wd_width >= 991){
			var topcart_form = $(".beaufly-topcart-desktop");
		}else{
			var topcart_form = $(".beaufly-topcart-mobile");
		}
	...
	function _click_add_to_cart(){
		...
		topcart_form.on('click', '.cart-icon', function(e){
			...
			if( !_body.hasClass("not-scroll") && $(".beaufly-topcart").hasClass("popup") ){
				_body.addClass("not-scroll");
			}			
		});
			...
			$.ajax({
				...
				success: function(data){
					...
					if (data.loggedin == true){
						...
						document.location.href = beaufly_ajax.redirecturl;
					}else{
	...
	function _click_quickview_button(){
		$('.quickview-button').on( "click", function(e) {
			...
			$.ajax({
				url: beaufly_ajax.ajaxurl,
				data: {
					"action" : "beaufly_quickviewproduct",
					'product_id' : product_id,
					"security" : beaufly_ajax.ajax_nonce
				},
				success: function(results) {
	...
	class Elementor_Js_Beaufly {
		static getInstance() {
			if (!Elementor_Js_Beaufly.instance) {
				Elementor_Js_Beaufly.instance = new Elementor_Js_Beaufly();
			}
			return Elementor_Js_Beaufly.instance;
		}
		constructor() {
	...
	Elementor_Js_Beaufly.getInstance();	
